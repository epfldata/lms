...
trait SimpleValExp extends SimpleValOps with BaseExp with EffectExp with VariablesExp {
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    ...
    case Reflect(NewSimpleVal(mk, name, defVal), u, es) => reflectMirrored(Reflect(NewSimpleVal(mtype(mk), f(name), f(defVal)), mapOver(f,u), f(es)))(mtype(manifest[A]),pos)
    case Reflect(e@SvGet(x), u, es) => reflectMirrored(Reflect(SvGet(f(x))(mtype(e.mK)), mapOver(f,u), f(es)))(mtype(manifest[A]),pos)
    case Reflect(SvUpdate(x, value), u, es) => reflectMirrored(Reflect(SvUpdate(f(x), f(value)), mapOver(f,u), f(es)))(mtype(manifest[A]),pos)
    case Reflect(SvMutable(x), u, es) => reflectMirrored(Reflect(SvMutable(f(x)), mapOver(f,u), f(es)))(mtype(manifest[A]),pos)
